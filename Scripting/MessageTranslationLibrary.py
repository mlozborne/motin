...
def makePowerStr(msg):
    ...
    lowByte, highByte = convertNaturalToBytes(len(st))
...
def ex(st):                        # expand the sting with length and checkSum
    n = len(st) + 1
    st += makeCheckSumByte(st)
    lowByte, highByte = convertNaturalToBytes(n)
    return lowByte + highByte + st
	
def ex2(st):                       # expand the sting WITHOUT checksum
    n = len(st)
    lowByte, highByte = convertNaturalToBytes(n)
    return lowByte + highByte + st
...
class TestMessageTranslationLibrary(unittest.TestCase):
    ...
    def testMakeMsgStr(self):
        ...
        self.assertEquals(ex("\x83"), makeMsgStr(msg))
        msg = PowerMsg(setting=kOff)
        self.assertEquals(ex("\x82"), makeMsgStr(msg))

        msg = LocoSpdMsg(slot=1, speed=2)
        self.assertEquals(ex("\xA0\x01\x02"), makeMsgStr(msg))
        msg = LocoSpdMsg(slot=1, speed=300)
        self.assertEquals(ex("\xA0\x01\x7F"), makeMsgStr(msg))

        msg = LocoDirfMsg(slot=1, direction=kBackward, lights=kOn, horn=kOn, bell=kOn)
        self.assertEquals(ex("\xA1\x01\x33"), makeMsgStr(msg))
        msg = LocoDirfMsg(slot=1, direction=kForward, lights=kOff, horn=kOff, bell=kOff)
        self.assertEquals(ex("\xA1\x01\x00"), makeMsgStr(msg))

        msg = LocoSndMsg(slot=1, mute=kOn, F5=kOn, F6=kOn)
        self.assertEquals(ex("\xA2\x01\x0B"), makeMsgStr(msg))
        msg = LocoSndMsg(slot=1, mute=kOff, F5=kOff, F6=kOff)
        self.assertEquals(ex("\xA2\x01\x00"), makeMsgStr(msg))

        msg = SwReqMsg(switch=1, direction=kClosed)
        self.assertEquals(ex("\xB0\x00\x30"), makeMsgStr(msg))
        msg = SwReqMsg(switch=1, direction=kThrown)
        self.assertEquals(ex("\xB0\x00\x10"), makeMsgStr(msg))
		
        msg = MoveSlotsMsg(slot1=1, slot2=2)
        self.assertEquals(ex("\xBA\x01\x02"), makeMsgStr(msg))
		
        msg = LocoAdrMsg(address=1111)
        st = "\xBF" + chr(1111 / 128) + chr(1111 % 128)
        self.assertEquals(ex(st), makeMsgStr(msg))
        ...
        st = chr(0) + chr(8) + chr(1111 % 128) + chr(1111 / 128) + \
            ...
            chr(2) + chr(0) + \
            chr(3) + chr(0)
        self.assertEquals(ex2(st), makeMsgStr(msg))
