...
--   4. Definition of types used by LayoutManagerPkg
   ...
   -- TYPE sensorNodePtr IS ACCESS SensorObj;
   TYPE SensorNode IS PRIVATE;
   TYPE SensorNodePtr IS ACCESS SensorNode;
   TYPE SensorNodeList IS PRIVATE;

--     TYPE SwitchObj IS PRIVATE;
--     TYPE switchNodePtr IS ACCESS SwitchObj;
   TYPE SwitchNode IS PRIVATE;
   TYPE SwitchNodePtr IS ACCESS SwitchNode;
   TYPE SwitchNodeList IS PRIVATE;
   
   type switchSettingNode is private;
   type switchSettingNodePtr is access switchSettingNode;
   type switchSettingList is private          ;
   

   TYPE blockingSection IS PRIVATE;
   TYPE blockingSectionPtr IS ACCESS blockingSection;
   TYPE blockingSectionList IS PRIVATE;
   ...
   type arraySensorNodePtrType         is array(positive range <>) of sensorNodePtr;
   type accessArraySensorNodePtrType   is access arraySensorNodePtrType;   
   PROCEDURE disposeArrayOfsensorNodePtr IS 
      NEW Ada.Unchecked_Deallocation(Object=>arraySensorNodePtrType, Name=>accessArraySensorNodePtrType);      
   ...
   PROTECTED TYPE LayoutManager IS
      ...
      -- Add a pointer to the sensor with Id = thrownSensor.id to CurrentSwitch
   ...
   PRIVATE
      ...
      CurrentSection : sectionNodePtr;
      CurrentSwitch  : SwitchNodePtr;
      -------------------- End data structures ------------------------- 
      -------------------------- 2e ------------------------------------
      ...
      -- IdentifyTrain, ReleaseReservation, RepositionTrain
       PROCEDURE ReleaseBlockings(BlockingList : blockingSectionList);
      ...
      PROCEDURE AddNewSensorToFront(TrainId : TrainIdType; Sensor : sensorNodePtr);
      FUNCTION  GetSensorPtrs (TrainId : TrainIdType) RETURN accessArraySensorNodePtrType;
      ...
      function  getSensorAtFrontOfReservedSectionPtr(section : sectionNode)   return sensorNodePtr;
      function  GetFrontSensorPtr(TrainId : TrainIdType)                     return sensorNodePtr;
      function  GetBackSensorPtr(TrainId : TrainIdType)                      return sensorNodePtr;
      function  getSensorAtFrontOfReservedSection(section : sectionNode)      return positive;
      ...
      function trainInSection(trainId : trainIdtype; sp : sectionNodePtr) return boolean;
      function sectionReserved(sp : sectionNodePtr) return boolean;
      function sectionOccupied(sp : sectionNodePtr) return boolean;
      procedure identifySensor(sx               : positive;               -- MO March 2014
                               ...
                               rightSectionPtr  : out sectionNodePtr);
      ...
      FUNCTION  IsSectionUseable(SectionPtr : sectionNodePtr) RETURN Boolean;
      ...
      PROCEDURE BlockSections (BlockingList : blockingSectionList);
      ...
      PROCEDURE GetSensor(SensorId : Positive; Sensor : OUT sensorNodePtr); 
      ...
      PROCEDURE GetFreeSection(SectList : sectionNodeList; OutSectPtr : OUT sectionNodePtr);
      PROCEDURE GetTrainSensorList(TrainId : TrainIdType; Sensors : OUT SensorNodeList);   
      
      -- MoveSwitch and MoveNextSwitch helpers   
      PROCEDURE GetSections(SwitchPtr  : switchNodePtr;
...
PRIVATE
   ...
   -- TYPE SwitchObj IS NEW LayoutObj WITH
      -- RECORD
         ...
         -- ThrownSensor  : sensorNodePtr;
      -- END RECORD;
      
   -- TYPE SwitchNode IS
      -- RECORD
         -- Switch : switchNodePtr;
         -- Next   : SwitchNodePtr;
      -- END RECORD;
   ...
   TYPE SwitchNode IS RECORD
         id            : positive;
         State         : SwitchStateType := Closed;
         TypeOfSwitch  : ControllerGlobals.SwitchType  := Normal;    --x Might be able to delete this 
                                                                     --x after switchSettingList is implemented
         ClosedSensors : SensorNodeList;             --x ditto
         NarrowSensors : SensorNodeList;             --x ditto
         ThrownSensor  : sensorNodePtr;              --x ditto. Will also be able to simplify the XML file.
         next          : switchNodePtr;
      END RECORD;
   ...
   TYPE blockingSection IS RECORD
         id           : positive;        --x id of the blocking section, can remove field
         sectionPtr   : sectionNodePtr;  --x this is the blocking section
         Next         : blockingSectionPtr;
   END RECORD;
      
   TYPE blockingSectionList IS RECORD
         Head : blockingSectionPtr;
         Tail : blockingSectionPtr;
   END RECORD;

   -- TYPE sectionNode IS NEW LayoutObj WITH
      -- RECORD
   ...
   type switchSettingNode is record
      switchPtr   :   switchNodePtr;                --x pointer to the switch
      setting     :   switchStateType := Closed;    --x required setting for switch
      next        :   switchSettingNodePtr;
   end record;
   
   type switchSettingList is record
      head   : switchSettingNodePtr;
      tail   : switchSettingNodePtr;
   end record;
   ...
   TYPE SectionNode IS RECORD
         ...
         IsUseable       : Boolean             := True;
         SensorList      : SensorNodeList;
         SwitchList      : SwitchNodeList;     --x Just a list of the switches in this section.
                                               --x Will be able to remove this after following field
                                               --x is functioning
         switchSetList   : switchSettingList;  --x A list of the switches and their required settings. 
                                               --x Build this as the XML file is read
                                               --x Will radically simplify routine for determining
                                               --x if a section is usable
         NextSectionList : sectionNodeList;
         PrevSectionList : sectionNodeList;
         BlockingList    : blockingSectionList;
         BlockCount      : Natural             := 0;
